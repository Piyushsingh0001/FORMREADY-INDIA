...
@Component({
  selector: 'app-signature-resizer',
  standalone: true,
  imports: [
    CommonModule, 
    FileUpload, 
  ...
  templateUrl: './signature-resizer.html',
  styleUrl: './signature-resizer.scss'
})
export class SignatureResizer {
  private imageService = inject(ImageProcessingService);

  selectedFile = signal<File | null>(null);
  croppedFile = signal<File | null>(null);
  isCropping = signal(false);
  isAutoCropping = signal(false);
  ...
  targetKb = signal<number>(20); // Default to 20KB for signatures
  ...
  async autoCrop() {
    ...
    this.isAutoCropping.set(true);
    try {
      const autoCropped = await this.imageService.autoCropSignature(file);
      this.selectedFile.set(autoCropped);
      // It updates the cropper automatically because selectedFile changed
    } catch (e: any) {
      this.errorMessage.set('Auto-crop failed: ' + e.message);
    } finally {
      this.isAutoCropping.set(false);
    }
  }
  ...
  async processImage() {
    ...
    try {
      ...
      const result = await this.imageService.compressImage(safeFile, {
        ...
        maxWidthOrHeight: 800 // Signatures rarely need to be larger than 800px
      });
      ...
      this.errorMessage.set(error.message || 'Failed to process signature');
    } finally {
  ...
  downloadResult() {
    ...
    a.href = url;
    a.download = `formready_sig_${result.file.name}`;
